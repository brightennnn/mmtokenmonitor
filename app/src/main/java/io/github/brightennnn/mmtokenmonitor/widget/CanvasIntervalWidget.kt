...
/**
 * 5小时用量 Widget — 竖条填满整个 widget
 * 用 ImageView + Canvas 画进度条，支持 Monet 动态颜色
 */
class CanvasIntervalWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        const val ACTION_REFRESH = "io.github.brightennnn.mmtokenmonitor.ACTION_REFRESH_WIDGET"

        fun updateAllWidgets(context: Context) {
            val manager = AppWidgetManager.getInstance(context)
            val ids = manager.getAppWidgetIds(
                android.content.ComponentName(context, CanvasIntervalWidget::class.java)
            )
        ...
        fun updateWidget(
            ...
            val pct = if (data.intervalTotal > 0) {
                ((data.intervalTotal - data.intervalUsed).toFloat() / data.intervalTotal * 100).toInt().coerceIn(0, 100)
            } else 0

            val views = RemoteViews(context.packageName, R.layout.canvas_interval_widget)
        ...
        private fun drawVerticalProgressBar(fillPercent: Int, accentColor: Int, bgColor: Int): Bitmap {
            // 2x2 widget 大约 180dp x 110dp，用较大分辨率确保清晰
            val width = 200
            val height = 300
